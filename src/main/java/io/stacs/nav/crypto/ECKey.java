...
import io.stacs.nav.crypto.jce.*;
...
public class ECKey implements Serializable {
    ...
    public ECKey(Provider provider, SecureRandom secureRandom) {
        ...
        if (pubKey instanceof BCECPublicKey) {
            ...
            throw new AssertionError(
                "Expected Provider " + provider.getName() + " to produce a subtype of ECPublicKey, found " + pubKey
                    .getClass());
        }
    }
    ...
    public ECKey(Provider provider, PrivateKey privKey, ECPoint pub) {
        ...
        if (privKey == null || isECPrivateKey(privKey)) {
            ...
            throw new IllegalArgumentException(
                "Expected EC private key, given a private key object with class " + privKey.getClass().toString()
    ...
    /**
     * Creates an ECKey that simply stacss the caller to ensure that point is really the result of multiplying the
    ...
    /**
     * Creates an ECKey that simply stacs the caller to ensure that point is really the result of multiplying the
     * generator point by the private key. This is used to speed things up when you know you have the right values
    ...
    public BigInteger keyAgreement(ECPoint otherParty) {
        if (privKey == null) {
            ...
            try {
                ...
                agreement.doPhase(
                    ECKeyFactory.getInstance(this.provider).generatePublic(new ECPublicKeySpec(otherParty, CURVE_SPEC)),
                    /* lastPhase */ true);
    ...
    /**
     * Recover address from signature byte [ ].
     *
     * @param recId       Which possible key to recover.
     * @param sig         the R and S components of the signature, wrapped.
     * @param messageHash Hash of the data that was signed.
    ...
    public static byte[] priKey2Bytes(PrivateKey privateKey) {
        if (privateKey == null) {
            return null;
    ...
    public static class ECDSASignature {
        ...
        /**
         * From components ecdsa signature.
         *
         * @param r -
         * @param s -
         * @param v -
        ...
        public static ECDSASignature decodeFromSignature(byte[] signature) {
            return ECDSASignature
                .fromComponents(Arrays.copyOfRange(signature, 1, 33), Arrays.copyOfRange(signature, 33, 65),
                    (byte)(signature[0] & 0xFF));
        }
        ...
        public byte[] toByteArray() {
            ...
            return ByteUtil.merge(new byte[] {fixedV}, ByteUtil.bigIntegerToBytes(this.r, 32),
     ...
     */
    @SuppressWarnings("serial") public static class MissingPrivateKeyException extends RuntimeException {
    }
